...
pthread_cond_t requestRemoved = PTHREAD_COND_INITIALIZER;
pthread_cond_t requestAdded = PTHREAD_COND_INITIALIZER;
pthread_mutex_t bufferLock = PTHREAD_MUTEX_INITIALIZER;

// make
// ./gunrock_web -p 8080 -t 8 -b 16
...
void *threadHandler(void * args){
  ...
  while(true){
    ...
    while (buffer.size() <= 0) {
      dthread_cond_wait(&requestAdded, &bufferLock);
    }
    client = buffer.front();
    buffer.pop_front();
    dthread_cond_signal(&requestRemoved);
...
int main(int argc, char *argv[]) {
  ...
  for(int i = 0; i < THREAD_POOL_SIZE; i++){
    dthread_create(&threads[i], NULL, threadHandler, NULL);
  }
  ...
  while(true) {
    ...
    while (int(buffer.size()) >= BUFFER_SIZE) {
      dthread_cond_wait(&requestRemoved, &bufferLock);
    }
    buffer.push_back(client);
    //delete client;
    dthread_cond_broadcast(&requestAdded);
    dthread_mutex_unlock(&bufferLock);
  }
}
