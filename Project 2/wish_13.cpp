...
int main(int argc, char *argv[]) {
  ...
  while(getline(file, cmd) || !(int(wishArgs.size()) == 1 && wishArgs[0] == "exit")){
    ...
    // in-built commands
      ...
      int pathNum = 0, fileDescriptor = -1;
      ...
      // found program in path
        ...
        // to implement: redirection/dup2
        // check for more than one file after redirection
        // should truncate & overwrite file if file already exists and create one if it doesn't exist

        pidBuilt = fork();

        // redirect to file
        if(pidBuilt == 0 && findString(wishArgs, ">", parStart) == int(wishArgs.size()) - 2){
          cout << wishArgs[int(wishArgs.size()) - 1] << endl;
        ...
        // more than 1 redirect arg -> error
        else if(pidBuilt == 0 && findString(wishArgs, ">", parStart) != -1){
          char error_message[30] = "An error has occurred\n";
          write(STDERR_FILENO, error_message, strlen(error_message)); 
          if(pidPar == 0)
            exit(1);
        ...
        // redirect STDOUT and STDERR to normal
        dup2(fileDescriptor, STDOUT_FILENO);
        dup2(fileDescriptor, STDERR_FILENO);
      }
    }
