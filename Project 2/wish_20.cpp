...
/* 
  ...
  TO DO:
    ...
    redirection (>), if output file exists overwrite (cmd + args > output), any errors rerouted to file output (dup2) -- done
...
// searches args from start index for symbol
// returns index of symbol if found, -1 if not found
...
int main(int argc, char *argv[]) {
  ...
  // loop until reached EOF (batch mode) or exit with no args
  while(getline(file, cmd) || (argc != 2 && !(int(wishArgs.size()) == 1 && wishArgs[0] == "exit"))){
    // interactive mode
    if(argc == 1){
      cout << "wish> ";
      getline(cin, cmd);
    ...
    // in-built commands
      ...
      // found program in path
        ...
        // redirect to file
        if(pidBuilt == 0 && int(wishArgs.size()) >= 3 && findString(wishArgs, ">", 0) == int(wishArgs.size()) - 2){
        ...
        else if(pidBuilt == 0 && findString(wishArgs, ">", 0) != -1 && findString(wishArgs, ">", 0) != int(wishArgs.size()) - 2){
          char error_message[30] = "An error has occurred\n";
          write(STDERR_FILENO, error_message, strlen(error_message)); 
          exit(1);
        }
